...
import { ArrowRight, Mail, Lock, Loader, Check, AlertCircle, UserCircle2 } from "lucide-react";
          ...
          <form onSubmit={handleSignup} className="space-y-6">
            <div className="space-y-4">
              ...
              <div className="relative group">
                <UserCircle2 
                  className={`absolute left-3 top-1/2 transform -translate-y-1/2 w-5 h-5 transition-colors duration-300 ${
                    focusedInput === 'role' ? 'text-blue-500' : 'text-gray-400'
                  }`}
                />
                <select
                  value={role}
                  ...
                  onBlur={() => setFocusedInput('')}
                  className={`w-full pl-12 pr-12 py-4 border-2 rounded-xl outline-none transition-all duration-300 appearance-none
                    ${focusedInput === 'role' ? 'border-blue-500 bg-blue-50/30' : 'border-gray-200 hover:border-gray-300'}`}
                  required
                >
