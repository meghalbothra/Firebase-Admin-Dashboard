...
export const AlertTitle = ({ className, children }: { className: string, children: React.ReactNode }) => {
  return <h3 className={`text-sm font-semibold leading-none ${className}`}>{children}</h3>;
};
    ...
    <Alert className={`relative overflow-hidden ${styles.container}`}>
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 space-y-1.5">
          <AlertTitle className={`text-sm font-semibold leading-none ${styles.title}`}>
          ...
          <AlertDescription className="space-y-2">
            ...
            <div className="flex items-center gap-1.5">
              <Clock className={`h-3.5 w-3.5 ${styles.timestamp}`} />
        ...
        <button 
          className={`rounded-full p-1 transition-colors hover:bg-white/20 ${styles.icon}`}
          aria-label="Dismiss"
        >
          <X className="h-4 w-4" />
        </button>
